...
def main():
	...
	for bot in bots:
		with open('newBehaviorsOrderRight.json') as f:
			order = json.load(f)
...
def runSingleBehavior(bot, behavior, outAngle, returnAngle):
	if behavior == "slow":
		...
		# goAtSpeed(bot, normalSpeed, outAngle, normalTime, returnSpeed, returnAngle, returnTime)
	# print time.time(), "called behavior"
	time.sleep(0.25)
	bot.set_rgb(0,0,00)
...
def goAtSpeed(bot, outSpeed, outAngle, outTime, returnSpeed, returnAngle, returnTime):
	# print time.time(), "in goAtSpeed, speed is", outSpeed
	...
	# bot.roll(0, outAngle, 0)
	# time.sleep(0.5)
	# bot.roll(outSpeed, outAngle, 1)
	# time.sleep(outTime)
	# bot.roll(0,0,0)
	# time.sleep(6 - 0.5 - outTime)

	# print time.time(), "end of goAtSpeed"
...
# def startBlink(bot, duration):
# 	on = True
# 	for quarter_second in range(0, 4 * duration):
# 		if on:
# 			bot.set_rgb(0,0,0)
# 			on = False
# 		else:
